...
LogEntry* SQLiteCommunicator::readLogentriesFromDatabase(char *binary_name,
		int count) {
	...
	if (sqlite3_prepare_v2(db, sqlStatement, -1, &compiledStatement, NULL)
		...
		int i = 0;
		while (sqlite3_step(compiledStatement) == SQLITE_ROW) {
			const unsigned char* id = sqlite3_column_text(compiledStatement, 0);
			const unsigned char* timestamp = sqlite3_column_text(
					compiledStatement, 1);
			const unsigned char* name = sqlite3_column_text(compiledStatement,
					2);
			double runtime = sqlite3_column_double(compiledStatement, 3);

			double size = sqlite3_column_double(compiledStatement, 4);
			double memory_usage = sqlite3_column_double(compiledStatement, 5);
			int exit_code = sqlite3_column_int(compiledStatement, 6);

			double max_memory_usage = sqlite3_column_double(compiledStatement,
					7);

			logEntries[i].createLogEntry((char *) name, runtime, size,
					memory_usage, max_memory_usage, exit_code);
...
int SQLiteCommunicator::addToTable(char *binary_name, LogEntry* logEntries_,
	...
	for (int var = 0; var < quantity; ++var) {
		...
		n =
				sprintf(
						tmp_s,
						"INSERT INTO %s  (\"name\" , \"runtime\", \"size\" , \"memory_usage\" , \"exit_code\", \"max_memory_usage\") VALUES ( \"%s\", %f, %f, %f, %d, %f)",
...
int SQLiteCommunicator::createTable(char *binary_name) {
	...
	n =
			sprintf(
					tmp_s,
					"create table '%s' (\"id\" INTEGER PRIMARY KEY  AUTOINCREMENT  NOT NULL  UNIQUE , \"timestamp\" DATETIME NOT NULL  DEFAULT CURRENT_TIMESTAMP, \"name\" VARCHAR NOT NULL , \"runtime\" DOUBLE DEFAULT null, \"size\" DOUBLE DEFAULT null, \"memory_usage\" DOUBLE DEFAULT null, \"exit_code\" INTEGER DEFAULT null, \"max_memory_usage\" DOUBLE DEFAULT null);",
					binary_name);
	sqlite3_exec(db, tmp_s, NULL, 0, &db_err);
	sqlite3_close(db);
